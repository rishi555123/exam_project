...
// ── Branch color map for print cards (inline styles — no Tailwind dependency)
const branchColors = {
...
// ── Generates the seating grid HTML for one room (used in both generate + reprint)
function buildSeatingPage(room_number, exam_date, exam_session, students) {
    const cards = students.map(s => {
        const c = branchColors[s.branch] || defaultColor;
        return `
        <div style="background:${c.bg};border:2px solid ${c.border};border-radius:12px;padding:8px 4px;height:90px;
                    display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;">
            <div style="font-size:10px;font-weight:900;color:#0f172a;font-style:italic;letter-spacing:-0.02em;">${s.roll_no}</div>
    ...
    <div class="a4-page">
        ...
        <div style="display:flex;justify-content:space-between;background:#0f172a;color:#fff;padding:14px 20px;border-radius:14px;margin-bottom:24px;font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:0.1em;font-style:italic;">
            <span>🏫 Hall: ${room_number}</span>
            <span>📅 ${exam_date}</span>
            <span>🕐 ${exam_session === 'FN' ? 'Forenoon' : 'Afternoon'}</span>
...
// ── Generates the attendance sheet HTML for one room
function buildAttendancePage(room_number, exam_date, exam_session, students) {
    const rows = students.map((s, idx) => `
        <tr style="border-bottom:1px solid #e2e8f0;">
            ...
            <td style="padding:8px 10px;font-size:9px;font-weight:900;color:${(branchColors[s.branch]||defaultColor).text};">${s.branch}</td>
    ...
    <div class="a4-page">
        <div style="border:5px double #0f172a;padding:20px;text-align:center;margin-bottom:20px;">
            <div style="font-size:10px;font-weight:900;letter-spacing:0.3em;text-transform:uppercase;color:#64748b;">Gokaraju Rangaraju Institute of Engineering and Technology</div>
            <div style="font-size:16px;font-weight:900;margin-top:8px;text-transform:uppercase;font-style:italic;">Attendance Sheet</div>
        </div>
        ...
        <table style="width:100%;border-collapse:collapse;border:2px solid #0f172a;">
            <thead>
                <tr style="background:#0f172a;color:#fff;">
                    <th style="padding:10px;font-size:9px;text-transform:uppercase;letter-spacing:0.15em;width:50px;">S.No</th>
                    <th style="padding:10px;font-size:9px;text-transform:uppercase;letter-spacing:0.15em;text-align:left;">Roll No</th>
                    <th style="padding:10px;font-size:9px;text-transform:uppercase;letter-spacing:0.15em;text-align:left;">Name</th>
                    <th style="padding:10px;font-size:9px;text-transform:uppercase;letter-spacing:0.15em;width:60px;">Dept</th>
                    <th style="padding:10px;font-size:9px;text-transform:uppercase;letter-spacing:0.15em;width:100px;">Signature</th>
                </tr>
            </thead>
            <tbody>${rows}</tbody>
        </table>
...
// ── Wraps pages in a printable HTML shell
function buildPrintShell(pagesHtml, backUrl = '/history') {
    return `<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>GRIET EMS — Seating Plan</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .np-btn {
            padding: 8px 24px; border-radius: 10px; font-size: 11px;
            font-weight: 900; font-style: italic; text-transform: uppercase;
            letter-spacing: 0.12em; cursor: pointer; border: none; text-decoration: none;
        }
        @media print {
            .no-print { display: none !important; }
            .a4-page { margin: 0; box-shadow: none; width: 100%; padding: 10mm; }
...
<body>
    <div class="no-print">
        ...
        <button onclick="window.print()" class="np-btn" style="background:#6366f1;color:#fff;box-shadow:0 4px 20px rgba(99,102,241,0.4);">🖨 Print All</button>
    </div>
    ${pagesHtml}
</body>
</html>`;
        ...
        if (studentsToPlace.length > totalCapacity) {
            return res.send(buildPrintShell(`
                <div class="a4-page" style="display:flex;align-items:center;justify-content:center;flex-direction:column;gap:16px;">
                    ...
                    <h2 style="color:#ef4444;font-size:24px;text-transform:uppercase;font-weight:900;">Insufficient Capacity</h2>
                    <p style="color:#64748b;">Students: <strong>${studentsToPlace.length}</strong> &nbsp;|&nbsp; Available Seats: <strong>${totalCapacity}</strong></p>
                </div>`, '/'));
        }
